...
- **Upload limits:** property photos max 2 per aggregate and listing photos max 5 per aggregate, 5 MB each (`image/jpeg`, `image/png`, `image/webp`); documents max 10 MB (`application/pdf`, `image/jpeg`, `image/png`). Enforced in `firebase/storage.rules` and the finalizing command handler. Media order is significant: the first accepted image is the primary/cover image. Newly selected client photos are auto-oriented, stripped of source metadata, bounded to 1920×1440, and recompressed as quality-82 JPEG before entering the offline aggregate, so uploads and retries reuse the smaller payload. Public listing copies are independently processed into bounded WebP before publication so a staged upload is never the public delivery asset.
