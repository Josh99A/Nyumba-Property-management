...
Public reads require `status == 'published'` and `expiresAt > request.time`. Browse queries must include those constraints and a page limit of at most 50; the composite index is provided in `firebase/firestore.indexes.json`.

The `expiresAt` constraint is the one place a client's own clock decides whether it may read at all. A query names an absolute cutoff, and Firestore rejects the *whole query* — not the offending rows — unless that cutoff is still ahead of the server's `request.time`. A device running behind the server therefore loses the entire marketplace on every cold start, which is precisely what a phone with automatic time off does. `FirestoreRemotePullGateway` answers a `permission-denied` by re-listening with a progressively later cutoff (5 minutes → 1 hour → 12 hours → 2 days). That cannot widen what is visible: the rule still re-checks every returned document against server time, so a later cutoff only withholds listings nearer their expiry, and the hourly `expirePublicListings` sweep is what actually retires them. The rule's contract is pinned by an emulator test in `firebase/functions/test/emulator/rules.test.ts`. Publication images live under a separate public Storage prefix and are generated only after validation. The worker auto-orients each source, strips embedded metadata, bounds it to 1920×1440, and encodes a WebP public derivative. That asynchronous media projection preserves the accepted cover-first order and increments the public projection version so version-aware clients replace the initial empty image list.
