...
| Listing | `listing.discard` | the owning active landlord or a Super Admin; permanently removes an **off-market** advert (draft or unpublished), deleting the private and public copies and queuing media cleanup. It refuses a `published` advert (unpublish first) and needs no audit reason, unlike the super-admin `listing.delete` moderation purge |
