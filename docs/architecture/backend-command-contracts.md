...
| Admin | `property.delete`, `unit.delete`, `listing.delete`, `document.purge` | **super-admin claim only**; permanent removal of records already out of circulation, across any landlord's workspace. Each refuses unless its target is archived (`isDeleted`) — or, for a listing, no longer `published` — so removal is always a deliberate second step. `property.delete` additionally refuses while any unit still references it, including archived ones, so purges run bottom-up. Deletion is a real Firestore delete, not a tombstone; storage objects follow via `purgeStorageObjects` / `cleanupListingMedia` / `purgeDocument` jobs. Counters are deliberately untouched: the archive already decremented them. Unlike the landlord command paths these load no landlord context, because cleaning up after suspended and unsubscribed accounts is the point |
...
| Listing | `listing.saveDraft/publish/unpublish` | owner or audited Admin/Super Admin acting for the canonical landlord; drafts accept at most five validated staged image paths in cover-first order; publication rechecks approval, availability, advertising entitlement, and moderation |
