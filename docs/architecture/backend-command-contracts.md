...
| Staff | `staff.invite`, `staff.claimInvite`, `staff.revoke`, `staff.updatePermissions` | owner invites a workspace member by email with a permission subset; seats capped by the plan's `staffSeatLimit` and coerced to the standard preset unless the plan allows custom roles; the invitee claims with a **verified email**, which creates a deterministic `staffMemberships/{landlordId}__{uid}` doc; revoke frees the seat and deletes the membership, cutting reads (Rules) and commands (`requireWorkspace`); invite/revoke/update are owner-only |
