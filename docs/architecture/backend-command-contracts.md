...
| Admin | `user.changeRole` | super-admin claim only, never self, never on an archived account; ordinary roles (`client|tenant|landlord`) only — administrator privileges are Auth custom claims granted exclusively by the ops script. Promotion to landlord provisions the missing landlord aggregates fail-closed (`pending` approval, `pending_payment` subscription); demotion leaves them in place as the audited record |
