...
| Admin | `landlord.approve`, `landlord.suspend`, `landlord.reinstate` | platform-admin or super-admin claim; mandatory reason and audit |
| Admin | `user.archive`, `user.restore` | platform-admin or super-admin claim, never self; mandatory reason and audit; versioned against `users/{uid}`. Archive marks the profile `archived`, disables the Auth account (background job), and unpublishes any landlord listings; restore reverses it. Open to any administrator precisely because it is reversible |
| Admin | `user.delete` | **super-admin claim only**, never self. Legal only from `archived`: it tombstones the profile (`isDeleted`, hidden from the directory) and deletes the Auth account via a background job. The claim boundary is irreversibility — an ordinary admin can take an account out of circulation but not destroy it |
| Admin | `user.changeRole` | platform-admin or super-admin claim, never self, never on an archived account; ordinary roles (`client`, `tenant`, or `landlord`) only — administrator privileges are Auth custom claims granted exclusively by the ops script, so no command can escalate anyone into them. Promotion to landlord provisions the missing landlord aggregates fail-closed (`pending` approval, `pending_payment` subscription); demotion leaves them in place as the audited record |
| Admin | `property.delete`, `unit.delete`, `listing.delete`, `document.purge` | **super-admin claim only**; permanent removal of records already out of circulation, across any landlord's workspace. Each refuses unless its target is archived (`isDeleted`) — or, for a listing, no longer `published` — so removal is always a deliberate second step. `property.delete` **cascades**: it deletes the archived property together with all of its rental spaces and their listings (private and public copies), queuing `cleanupListingMedia` per listing. Direct `unit.delete` performs the same listing cleanup for that unit, preventing orphaned drafts. Because a property can only be archived once every unit is archived (vacant and unadvertised), the cascade never reaches an occupied space or a live advert and touches no counter — it re-checks those invariants and refuses if any fail. It also refuses a portfolio too large for Firestore's 500-write transaction limit (`portfolioTooLargeToCascade`); a background-job fan-out is the required scale-up path rather than weakening atomicity. Tenancy, lease, payment, receipt, document, maintenance and notice history is deliberately **kept**: this is a portfolio cleanup, not an accounting erasure. Deletion is a real Firestore delete, not a tombstone; storage objects follow via `purgeStorageObjects` / `cleanupListingMedia` / `purgeDocument` jobs. Unlike the landlord command paths these load no landlord context, because cleaning up after suspended and unsubscribed accounts is the point |
| Portfolio | `property.create/update/archive`, `unit.create/update/archive/restore` | owning landlord or audited Admin/Super Admin acting for a canonical target landlord; account/entitlement checks; property create/update accepts at most two validated staged image paths in display order (first is primary) and refuses an active property with the same landlord, name, street address, and city so a fresh command ID cannot record the same form twice; unit counter transaction. Landlords may set vacant/reserved/maintenance/inactive availability when no lease is active; `occupied` is tenancy-managed |
...
| Staff | `staff.invite`, `staff.claimInvite`, `staff.revoke`, `staff.updatePermissions` | owner invites a workspace member by email with a permission subset; seats are capped transactionally through `landlordAccounts.activeStaffSeatCount` and the plan's `staffSeatLimit`, and permissions are coerced to the standard preset unless the plan allows custom roles; a verified invitee may hold one active landlord membership only, and ambiguous multi-workspace claims fail closed; revoke decrements the counter and deletes the membership, cutting reads (Rules) and commands (`requireWorkspace`); invite/revoke/update are owner-only |
...
| Billing | `payment.declare` | the **tenant** on an active lease reports rent they already paid. A mandatory `reference` is the proof the landlord judges it on. Records a claim only: no allocation, no invoice settled, no balance moved, no receipt — otherwise a tenant could clear their own arrears by asserting them away. Lease authority comes from `leases.tenantUserUid`, never the payload |
| Billing | `payment.confirmDeclared` / `payment.rejectDeclared` | the owning landlord accepts or refuses a declared payment. Confirming settles it through the same helper as `payment.recordAgainstTenancy`, so allocations, receipt and balances are identical either way, and the tenant's proof is kept as the payment reference. Rejecting closes the claim with a mandatory `reasonCode` (+ optional note), unwinds nothing (nothing had settled), keeps the record for dispute history, and notifies the tenant |
...
| Listing | `listing.saveDraft/publish/unpublish` | owner or authorized workspace staff, or audited Admin/Super Admin acting for the canonical landlord; drafts accept at most five validated staged image paths in cover-first order, and a unit can have only one private listing aggregate at a time (discard it before creating a replacement). The server derives and persists `propertyId` from the canonical unit and `currency` from market configuration so private listing documents always satisfy the client read contract; publication rechecks approval, availability, advertising entitlement, and moderation |
| Listing | `listing.discard` | the owning active landlord only (including a Super Admin only when acting as that listing's owning landlord); permanently removes an **off-market** advert (draft or unpublished), deleting the private and public copies and queuing media cleanup. It refuses a `published` advert (unpublish first). Cross-landlord Super Admin removal must use the reasoned `listing.delete` moderation purge, so this no-reason self-service path cannot bypass the audit contract |
...
| Subscription | `subscription.confirmPayment` | Admin/Super Admin only, never self; the audited transition to `active`. It is the manual (cash) confirmation, and the same transition the electronic aggregator's **signed provider webhook** will call to auto-activate a `mobile_money`/`card` upgrade with no admin. Those two paths do not overlap: an upgrade still sitting `awaiting_payment` is rejected with `electronicUpgradeAwaitingProvider` rather than adopted by an administrator, so a plan can never open on a checkout the provider never confirmed. Passing `tier` explicitly is the audited override for rescuing a failed provider callback by hand. Also approves a still-`pending` landlord account (`approvalReasonCode: PAYMENT_CONFIRMED`) in the same transaction; rejects over a `suspended` account so payment never undoes a suspension. On an already-`active` subscription it applies a paid plan change instead (explicit `tier` or the landlord's `requestedTier`, clearing the request and its billing channel, recording `planChangedAt`); an active subscription with no tier change still rejects |
| Subscription | `subscription.rejectPayment` | Admin/Super Admin only, never self; records that staff could not confirm a reported payment. Clears the pending request with a mandatory `reasonCode` (and optional note relayed to the landlord); the tier, status, entitlements and paid period are all left untouched. Rejects when nothing is awaiting review |
| Subscription | `subscription.downgrade` | Admin/Super Admin only, never self; moves an active subscription to a smaller plan with no payment. **Downward only**, judged against the server-owned `unitLimit` (not a hard-coded tier order) so it stays correct after `plan.update` re-scopes plans, and so an admin session can never grant paid capacity for free. Clears any pending upgrade, leaves the paid period alone, and preserves downgrade safety: nothing deleted, tenants unaffected, only new units/listings held while over the limit |
| Subscription | `subscription.deactivate` | Admin/Super Admin only, never self; sets `canceled`, which locks the workspace because every landlord command requires `active`. Preserve-everything semantics: no deletions, listings untouched, tenants keep full portal access. Reversed by `confirmPayment`. Compliance suspension remains `landlord.suspend` |
| Subscription | `plan.update` | platform-admin or super-admin claim; edits one existing `planCatalog/{tier}` (prices, presentation, feature `implemented` flags, visibility) and mirrors changed limits into `backendConfig/entitlements` in the same transaction so display and enforcement cannot drift. Tier travels in the payload (`expectedCatalogVersion` is the concurrency token) because tier IDs are shorter than the envelope aggregateId pattern. Rejects a yearly price above twelve monthly payments |
| Support | `support.open`, `support.reply`, `support.updateStatus` | landlord ↔ platform-admin conversation on `supportTickets/{id}` (docs/architecture/support-desk-plan.md). Deliberately **not** gated on approval or an active subscription: `requireActiveLandlord` would reject `past_due`, `pending_payment` and suspended accounts, which is exactly the set of landlords who most need to reach a human. Gate is a live user account plus an existing landlord account; approval and subscription state are recorded on the ticket as context instead. Priority is server-derived, never sent. `reply` resolves the author from the ticket (owner → `landlord`, else `requirePlatformAdmin` → `support`) and advances the status itself, so nobody sets a state to have a conversation. `updateStatus` is admin-driven except two landlord transitions: closing their own ticket, and reopening a `resolved` one inside `SUPPORT_REOPEN_WINDOW_DAYS`. Staff are excluded throughout |
| Communication | `platform.broadcast` | platform-admin or super-admin claim; records a `platformBroadcasts/{id}` announcement targeted at `all_users`, a role group (`landlords`/`tenants`/`clients`), one subscription `tier`, or one `user`, then hands delivery (notification inbox + push + email copy) to the durable `broadcastFanout` job. Scoped audiences are validated up front — unknown tiers fail closed, missing/deleted users reject with NOT_FOUND |

The operational command groups above (Portfolio, Tenancy, Billing,
Maintenance, Communication, Listing, Reporting, Documents) accept **either the
owning landlord or a granted staff member**: `requireWorkspace`
(`shared/accounts.ts`) resolves the actor to the owner's workspace and
authorizes the capability that maps to that group. Owner-only commands
(subscription, plan, staff management, account lifecycle) keep the stricter
`requireActiveLandlord` and are never staff-reachable. See
docs/architecture/subscription-tiers.md for the staff seat model.
...
Admin commands require either the `platformAdmin` or `superAdmin` custom claim plus recent authentication for destructive/high-impact actions.

The line between the two claims is **reversibility**, not sensitivity. An ordinary administrator runs the platform day to day: approvals, suspensions, subscription decisions, plan pricing, announcements, role changes, and account archive/restore — every one of which can be undone. The `superAdmin` claim adds only the irreversible operations: `user.delete` and the `property.delete` / `unit.delete` / `listing.delete` / `document.purge` purges. Self-promotion, self-suspension, acting on your own account, and client-granted claims are forbidden to both, and no command can mint an administrator: the privileged claims come only from the audited ops script. Approval/suspension reasons are enumerated, and every action records actor, target, source IP metadata where legally appropriate, and a redacted diff. The detailed matrix is in [role-permissions.md](role-permissions.md).
